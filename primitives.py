...
class Shape(ABC):
    ...
    def rand_pos(self):
        ...
        if self.length == self.width:
            coord_1 = choice([random.uniform(0.2, 0.85), random.uniform(-0.85, -0.2)])
            coord_2 = choice([random.uniform(0.2, 0.85), random.uniform(-0.85, -0.2)])
...
class Box(Shape):
    ...
    def show(self):
        ...
        tree.find('xacro:property[@name="length"]').set("value", str(self.length))
        tree.find('xacro:property[@name="width"]').set("value", str(self.width))
        tree.find('xacro:property[@name="height"]').set("value", str(self.height))
...
class Sphere(Shape):
    ...
    def show(self) -> None:
        ...
        tree.find('xacro:property[@name="use_cylinder"]').set("value", "false")
        tree.find('xacro:property[@name="radius"]').set("value", str(self.radius))

        tree.write("shape.urdf.xacro")


class Cylinder(Shape):
    def __init__(self, radius=0, mass=10) -> None:
        super().__init__(mass)
        self.radius = radius
