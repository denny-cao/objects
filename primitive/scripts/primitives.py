...
def normal(self, max):
    # Get a random number from a truncated normal distribution

    mu = (MIN_DIM + max) / 2 # Set center at midpoint of min and max values
    sigma = (max - mu) / 8

    return truncnorm(a=MIN_DIM, b=max, loc=mu, scale=sigma)
...
class Shape(object):
    ...
    def rand_pos(self, msg):
        ...
        self.x = round(rng.uniform(-MAX_COORD, MAX_COORD), ndigits=4)
        ...
        if self.lower_bound_x <= self.x <= self.upper_bound_x:
            ...
            if rng.choice([0, 1]) and self.upper_bound_y < MAX_COORD - MIN_DIM:
                self.y = rng.uniform(self.upper_bound_y, MAX_COORD)
            
            else:
                self.y = rng.uniform(-MAX_COORD, self.lower_bound_y)
        else:
            self.y = round(rng.uniform(-MAX_COORD, MAX_COORD), ndigits=4)
...
class Box(Shape):
    ...
    def rand_dim(self):
        ...
        '''
        self.width = round(rng.uniform(MIN_DIM, self.max_dim_x), ndigits=4)
        self.length = round(rng.uniform(MIN_DIM, self.max_dim_y), ndigits=4)
        self.height = round(rng.uniform(MIN_DIM, self.largest_z), ndigits=4)
        '''

        self.width = round(normal(self.max_dim_x), ndigits=4)
        self.length = round(normal(self.max_dim_y), ndigits=4)
        self.length = round(normal(self.largest_z), ndigits=4)
...
class Sphere(Shape):
    ...
    def rand_dim(self):
        ...
        self.radius = round(normal(max_rad), ndigits=4)
        '''
        self.radius = round(rng.uniform(MIN_DIM, max_rad), ndigits=4)
        '''
        self.z = self.radius
...
class Cylinder(Shape):
    ...
    def rand_dim(self):
        ...
        '''
        self.radius = round(rng.uniform(MIN_DIM, max_rad), ndigits=4)
        self.length = round(rng.uniform(MIN_DIM, self.largest_z), ndigits=4)
        '''
